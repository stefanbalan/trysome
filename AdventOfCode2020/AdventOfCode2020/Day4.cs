...
namespace AdventOfCode2020
{
    public class Day4
    {
        ...
        {
            var passports = input.Split("\r\n\r\n");
            return passports.Count(TestPassport1);
        }

        private bool TestPassport1(string p)
        {
            var pass = p.Replace("\r\n", " ") + " ";
            var byr = new Regex(@"byr:(19[2-9]\d|200[0-2])\s");  //byr(Birth Year) - four digits; at least 1920 and at most 2002.
            var iyr = new Regex(@"iyr:(201\d|2020)\s");  //iyr(Issue Year) - four digits; at least 2010 and at most 2020.
            var eyr = new Regex(@"eyr:(202\d|2030)\s");  //eyr(Expiration Year) - four digits; at least 2020 and at most 2030.
            var hgt = new Regex(@"hgt:(1[5-8]\dcm|19[0-3]cm|59in|6\din|7[0-6]in)\s");  //hgt(Height) - a number followed by either cm or in: If cm, the number must be at least 150 and at most 193.If in, the number must be at least 59 and at most 76.
            var hcl = new Regex(@"hcl:(#[0-9a-fA-F]{6})\s");  //hcl(Hair Color) - a # followed by exactly six characters 0-9 or a-f.
            var ecl = new Regex(@"ecl:(amb|blu|brn|gry|grn|hzl|oth)\s");  //ecl(Eye Color) - exactly one of: amb blu brn gry grn hzl oth.
            var pid = new Regex(@"pid:(\d{9})\s"); //pid(Passport ID) - a nine - digit number, including leading zeroes.
            //pass.Contains("cid") &&   //cid(Country ID) - ignored, missing or not.

            var mbyr = byr.Match(pass);
            var miyr = iyr.Match(pass);
            var meyr = eyr.Match(pass);
            var mhgt = hgt.Match(pass);
            var mhcl = hcl.Match(pass);
            var mecl = ecl.Match(pass);
            var mpid = pid.Match(pass);
        ...
        private bool TestPassport2(string pass)
        {
            var byr = new Regex(@"byr:(\d{4})");  //(Birth Year)                            byr(Birth Year) - four digits; at least 1920 and at most 2002.
            var iyr = new Regex(@"iyr:(\d{4})");  //(Issue Year)                            iyr(Issue Year) - four digits; at least 2010 and at most 2020.
            var eyr = new Regex(@"eyr:(\d{4})");  //(Expiration Year)                       eyr(Expiration Year) - four digits; at least 2020 and at most 2030.
            var hgt = new Regex(@"hgt:(\d{3}cm|\d{2}in)");  //(Height)                      hgt(Height) - a number followed by either cm or in: If cm, the number must be at least 150 and at most 193.If in, the number must be at least 59 and at most 76.
            var hcl = new Regex(@"hcl:(#[0-9a-fA-F]{6})");  //(Hair Color)                  hcl(Hair Color) - a # followed by exactly six characters 0-9 or a-f.
            var ecl = new Regex(@"ecl:(amb|blu|brn|gry|grn|hzl|oth)");  //(Eye Color)       ecl(Eye Color) - exactly one of: amb blu brn gry grn hzl oth.
            var pid = new Regex(@"pid:(\d{9})"); //(Passport ID)                            pid(Passport ID) - a nine - digit number, including leading zeroes.
            //pass.Contains("cid") &&   //(Country ID)                                      cid(Country ID) - ignored, missing or not.
            ...
            if (!mbyr.Success) return false;
            var nbyr = int.Parse(mbyr.Groups[1].Value);
            if (nbyr < 1920 || nbyr > 2002) return false;

            if (!miyr.Success) return false;
            var niyr = int.Parse(mbyr.Groups[1].Value);
            if (niyr < 2010 || niyr > 2020) return false;

            if (!meyr.Success) return false;
            var neyr = int.Parse(mbyr.Groups[1].Value);
            if (neyr < 2020 || neyr > 2030) return false;


            if (!mhgt.Success) return false;
            if (!mhcl.Success) return false;
            if (!mecl.Success) return false;
            if (!mpid.Success) return false;
